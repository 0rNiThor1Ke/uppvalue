...
const V={bg:"#121212",card:"#1E1E1E",card2:"#252525",card3:"#2C2C2C",border:"#363636",green:"#00C805",red:"#DC3545",blue:"#5865F2",gold:"#FFD700",txt:"#FFFFFF",txM:"#B0B0B0",txD:"#666666"};
const TIER_C={1:V.green,2:V.gold,3:V.blue};
const TIER_L={1:"Achat Immediat",2:"Surveillance",3:"Long Terme"};
const DOMAINS={AMZN:"amazon.com",MSFT:"microsoft.com",AAPL:"apple.com",GOOGL:"google.com",META:"meta.com",NVDA:"nvidia.com",TSLA:"tesla.com","SU.PA":"se.com","AI.PA":"airliquide.com","MC.PA":"lvmh.com",SPGI:"spglobal.com",MA:"mastercard.com",FTNT:"fortinet.com",MELI:"mercadolibre.com",CCJ:"cameco.com",GLD:"spdrgoldshares.com","BTC-USD":"bitcoin.org","ETH-USD":"ethereum.org",CRCL:"circle.com",COIN:"coinbase.com",ADBE:"adobe.com",AMD:"amd.com"};
...
function Logo({ticker,size=32}){
  const[err,setErr]=useState(false);
  if(err)return<div style={{width:size,height:size,borderRadius:8,background:V.card3,display:"flex",alignItems:"center",justifyContent:"center",fontSize:size*.4,fontWeight:700,color:V.txM}}>{ticker[0]}</div>;
  return<img src={getLogo(ticker)} width={size} height={size} style={{borderRadius:8,objectFit:"contain",background:V.card3}} onError={()=>setErr(true)} alt=""/>;
}
...
export default function UppValueApp(){
  ...
  useEffect(()=>{try{const s=localStorage.getItem("uppvalue-v4");if(s){const d=JSON.parse(s);if(d.portfolio)setPortfolio(d.portfolio);if(d.watchlist)setWatchlist(d.watchlist);}}catch(e){}},[]);
  const save=useCallback((p,w)=>{localStorage.setItem("uppvalue-v4",JSON.stringify({portfolio:p??portfolio,watchlist:w??watchlist}));},[portfolio,watchlist]);
  ...
  return(<div style={{minHeight:"100vh",background:V.bg,color:V.txt,fontFamily:"Inter,sans-serif",display:"flex"}}>
    {/* SIDEBAR */}
    <div style={{width:220,minHeight:"100vh",background:V.card,borderRight:"1px solid "+V.border,padding:"14px 8px",display:"flex",flexDirection:"column",position:"sticky",top:0,flexShrink:0}}>
      <div style={{padding:"6px 12px 18px",display:"flex",alignItems:"center",gap:8}}>
        <span style={{color:V.green,fontSize:20}}>&#9650;</span>
        <span style={{fontSize:18,fontWeight:800,letterSpacing:"-1px"}}>UppValue</span>
      </div>
      <button onClick={()=>{setShowAdd(true);setAddMode("portfolio");}} style={{...bg,width:"calc(100% - 16px)",justifyContent:"center",margin:"0 8px 12px"}}>+ Ajouter</button>
      <nav style={{flex:1,display:"flex",flexDirection:"column",gap:1}}>
        {[{id:"portfolio",l:"Portefeuille"},{id:"watchlist",l:"Watchlist"},{id:"charts",l:"Graphiques"},{id:"sp500",l:"vs S&P 500"},{id:"dcf",l:"Valorisation"}].map(n=>
          <button key={n.id} onClick={()=>setTab(n.id)} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",borderRadius:8,border:"none",width:"100%",background:tab===n.id?V.green+"20":"transparent",color:tab===n.id?V.green:V.txM,cursor:"pointer",fontSize:13,fontWeight:tab===n.id?600:400,textAlign:"left"}}>{n.l}</button>
        )}
      </nav>
      <button onClick={refreshAll} disabled={loading} style={{display:"flex",alignItems:"center",gap:6,padding:"10px 14px",borderRadius:8,border:"1px solid "+V.border,background:"transparent",color:loading?V.txD:V.txM,cursor:loading?"wait":"pointer",fontSize:12,width:"calc(100% - 16px)",margin:"0 8px 8px"}}>{loading?"Sync...":"Rafraichir"}</button>
    </div>

    {/* MAIN */}
    <div style={{flex:1,padding:"20px 24px",maxWidth:920,overflow:"auto"}}>
      ...
      {showAdd&&<div style={{position:"fixed",inset:0,background:"#000c",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000}} onClick={()=>setShowAdd(false)}>
        <div onClick={e=>e.stopPropagation()} style={{width:440,background:V.card,border:"1px solid "+V.border,borderRadius:16,padding:24}}>
          ...
          <div style={{display:"flex",gap:8,marginBottom:16}}>
            ...
            <button onClick={handleFetch} disabled={addFetching} style={{...bg,opacity:addFetching?.5:1}}>Go</button>
          </div>
        ...
        {portfolio.length===0?<div style={{...cs,textAlign:"center",padding:48,color:V.txD}}>Ajoutez votre premiere position</div>:<>
          ...
          {enrichedP.map(e=><div key={e.id} style={{...cs,padding:14}}>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              ...
              <button onClick={()=>{const p=portfolio.filter(x=>x.id!==e.id);setPortfolio(p);save(p,null);}} style={{background:"transparent",border:"none",color:V.txD,cursor:"pointer",marginLeft:4,fontSize:16}}>X</button>
            </div>
          </div>)}
        </>}
      </div>}
      ...
      {tab==="watchlist"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        {watchlist.length===0?<div style={{...cs,textAlign:"center",padding:48,color:V.txD}}>Ajoutez des tickers a surveiller</div>:
          [1,2,3].map(tier=>{const items=enrichedW.filter(w=>w.tier===tier);if(!items.length)return null;return<div key={tier}>
            ...
            {items.map(w=><div key={w.id} style={{...cs,padding:14,marginBottom:8,borderLeft:"3px solid "+TIER_C[w.tier]}}>
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <div style={{flex:1}}>
                  <div style={{fontWeight:700,fontSize:14}}>{w.name} <span style={{fontSize:10,color:V.txD,fontFamily:"monospace"}}>{w.ticker}</span>
                    {w.atTarget&&<span style={{fontSize:8,padding:"2px 6px",borderRadius:3,background:V.green+"20",color:V.green,fontWeight:700,marginLeft:6}}>PRIX CIBLE ATTEINT</span>}
                  </div>
                </div>
                <div style={{textAlign:"right"}}>
                  {w.live&&<div style={{fontSize:17,fontWeight:800,fontFamily:"monospace"}}>{fmtU(w.live)}</div>}
                  <div style={{fontSize:11,color:V.txD}}>Cible: <span style={{color:V.gold,fontFamily:"monospace"}}>{w.tp>0?fmtU(w.tp):"---"}</span>
                    {w.gap!=null&&<span style={{color:w.gap<=0?V.green:V.red,marginLeft:4}}>({fmtP(w.gap)})</span>}
                  </div>
                </div>
      ...
      {tab==="charts"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
          {[...portfolio,...watchlist].map(p=><button key={p.id} onClick={()=>setSelChart(p.ticker)} style={{padding:"6px 12px",borderRadius:6,border:selChart===p.ticker?"1px solid "+V.green:"1px solid "+V.border,background:selChart===p.ticker?V.green+"20":"transparent",color:selChart===p.ticker?V.green:V.txM,cursor:"pointer",fontSize:11,fontWeight:selChart===p.ticker?600:400}}>{p.name}</button>)}
        </div>
        {chartData?<div style={cs}>
          <div style={{fontSize:14,fontWeight:600,marginBottom:4}}>{chartData.name} - 1 an</div>
          <div style={{fontSize:12,color:V.txD,marginBottom:12}}>Prix: <span style={{color:V.txt,fontFamily:"monospace"}}>{fmtU(chartData.price)}</span> | Rendement: <span style={{color:chartData.returnPct>=0?V.green:V.red}}>{fmtP(chartData.returnPct)}</span></div>
          ...
          {chartData.comparison&&chartData.comparison.length>0&&<div>
            <div style={{fontSize:11,color:V.txD,marginBottom:6}}>Performance vs S&P 500 (% depuis le debut)</div>
            <ResponsiveContainer width="100%" height={250}>
        ...
        </div>:<div style={{...cs,textAlign:"center",padding:48,color:V.txD}}>Selectionnez un ticker pour voir son graphique</div>}
      </div>}

      {/* S&P TABLE */}
      {tab==="sp500"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
        <h2 style={{margin:0,fontSize:18,fontWeight:700,color:V.green}}>Opportunity Cost vs S&P 500</h2>
        {enrichedP.length===0?<div style={{...cs,textAlign:"center",padding:48,color:V.txD}}>Ajoutez des positions</div>:
          <div style={cs}><table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr>{["","Position","PRU","Prix","P&L","P&L %",""].map(h=><th key={h} style={{padding:8,textAlign:"left",color:V.txD,fontSize:10,borderBottom:"1px solid "+V.border}}>{h}</th>)}</tr></thead>
            <tbody>{enrichedP.map(e=><tr key={e.id} style={{cursor:"pointer"}} onClick={()=>{setSelChart(e.ticker);setTab("charts");}}>
              <td style={{padding:8}}><Logo ticker={e.ticker} size={24}/></td>
              <td style={{padding:8,fontWeight:600}}>{e.name}</td>
              <td style={{padding:8,fontFamily:"monospace"}}>{fmtU(e.pru)}</td>
              <td style={{padding:8,fontFamily:"monospace"}}>{e.live?fmtU(e.live):"---"}</td>
              <td style={{padding:8,fontFamily:"monospace",color:e.pl!=null?(e.pl>=0?V.green:V.red):V.txD}}>{e.pl!=null?fmt(e.pl):"---"}</td>
              <td style={{padding:8,fontFamily:"monospace",color:e.plP!=null?(e.plP>=0?V.green:V.red):V.txD}}>{fmtP(e.plP)}</td>
              <td style={{padding:8,fontSize:10,color:V.blue}}>Voir graphique</td>
            </tr>)}</tbody>
          </table></div>}
      </div>}
      ...
      <div style={{marginTop:28,padding:"14px 0",borderTop:"1px solid "+V.border,textAlign:"center"}}><p style={{margin:0,fontSize:10,color:V.txD}}>UppValue v4 | Yahoo Finance | localStorage</p></div>
    </div>
  </div>);
}